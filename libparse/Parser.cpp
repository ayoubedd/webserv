...
bool libparse::checkConfig(const std::string &fileName, libparse::Config &config) {
  ...
  if (!res.first) {
    std::cout << "Error Token : " << res.second << std::endl;
    return res.first;
  }
  res = checkFileExist(config);
  if (!res.first) {
    std::cout << "Error: File Does Not Exist or Executable: " << res.second << std::endl;
    return res.first;
  }
  res = checkDefaultRout(config);
  if (!res.first) {
    std::cout << "Error Default Route at: " << res.second << std::endl;
    return res.first;
  }
  res = checkDuplicatePort(config);
  if (!res.first) {
    std::cout << "Error Dulitple Port at " << res.second << std::endl;
    return res.first;
  }
  return true;
}
